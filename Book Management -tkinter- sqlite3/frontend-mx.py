...
@Marshall, 2018/01
Original code: 
    Udemy - Mega Applications 5 for Python
...
Run this program with backend.py and books.db under same folder -- and play around!
'''
...
def search_command():
    ...
    if (len(results) == 0):
        list1.insert(END, [0,"No results","",0,0])
    else:
        for row in results:
            list1.insert(END,row)
...
list1.bind('<<ListboxSelect>>', get_selected_row) # important
...
window.bind_all('<KeyPress>', esc)  # press esc to close - part 2 
